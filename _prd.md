...
- **Distribution**: GitHub repo `thomas-schenkelberg/claude-code-wrap-up-plugin` acts as the marketplace; users `add` it then `install wrap-up@wrap-up`. (Repo history: created at `tschenkster/claude-code-wrap-up-skill` on 2026-05-01, renamed to `…-plugin` on 2026-05-12, transferred to the `thomas-schenkelberg` GitHub Org on 2026-05-13. GitHub redirects every prior URL automatically — existing installs keep working.)
